...
export class CatchLegendaryRouletteComponent implements OnInit {
  ...
  catchRate = [
    ...
    { text: 'nein', fillStyle: 'crimson', weight: 3 }
  ];

  ngOnInit(): void {
    if (this.currentRound >= 8) {
      this.catchRate = [
        { text: 'ja', fillStyle: 'green', weight: 3 },
        { text: 'nein', fillStyle: 'crimson', weight: 1 },
      ];
    } else if (this.currentRound >= 4) {
      this.catchRate = [
        { text: 'ja', fillStyle: 'green', weight: 2 },
        { text: 'nein', fillStyle: 'crimson', weight: 2 },
      ];
    }
  }
  ...
  onItemSelected(index: number): void {
    if (this.catchRate[index].text === 'ja') {
